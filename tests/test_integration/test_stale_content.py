...
class TestExtractWorker(unittest.TestCase):
    ...
    def test_extraction_of_non_extracted(self):
        ...
        bibcode, ft_source, provider = lines[0].strip().split("\t")
        test_expected = check_if_extract.create_meta_path(
            {"bibcode": bibcode}, extract_key='FULLTEXT_EXTRACT_PATH_UNITTEST')
        ...
        test_meta_content = {"index_date": datetime.utcnow().isoformat()+'Z', "bibcode": bibcode,
                             "provider": provider, 'ft_source': ft_source}
        with open(test_expected, 'w') as test_meta_file:
            json.dump(test_meta_content, test_meta_file)

        with open(test_expected.replace('meta.json', 'fulltext.txt'), 'w') as test_full_text_file:
            test_full_text_file.write('Full text content')
        time.sleep(1)
        with open(ft_source, 'w') as temp_write:
            temp_write.write('')
